...
export default function Navigation() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#0A0A0A]/95 backdrop-blur-sm border-b border-[#222222]">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-[#DC2626] text-xs font-medium">Cabling & Infrastructure</span>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <div className="relative group">
              ...
              {servicesOpen && (
                <div
                  className="absolute top-full left-0 bg-[#141414] border border-[#222222] rounded-lg shadow-xl py-2 w-56 z-50"
                  onMouseEnter={() => setServicesOpen(true)}
                  onMouseLeave={() => setServicesOpen(false)}
                >
                  {services.map((s) => (
                    <Link key={s.href} href={s.href} className="block px-4 py-2 text-sm text-gray-300 hover:text-white hover:bg-[#222222] transition-colors">
                      {s.label}
                    </Link>
                  ))}
                </div>
            ...
            <div className="relative group">
              ...
              {industriesOpen && (
                ...
                >
                  {industries.map((i) => (
                    <Link key={i.href} href={i.href} className="block px-4 py-2 text-sm text-gray-300 hover:text-white hover:bg-[#222222] transition-colors">
                      {i.label}
                    </Link>
                  ))}
                </div>
            ...
            <Link href="/get-a-quote/" className="bg-[#DC2626] hover:bg-red-600 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors">
              Get a Quote
            </Link>
          </div>
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="md:hidden bg-[#141414] border-t border-[#222222] px-4 py-4 space-y-3">
          <p className="text-xs font-semibold text-[#DC2626] uppercase tracking-wider">Services</p>
          {services.map((s) => (
            <Link key={s.href} href={s.href} className="block text-sm text-gray-300 hover:text-white py-1" onClick={() => setMobileOpen(false)}>
              {s.label}
            </Link>
          ))}
          <p className="text-xs font-semibold text-[#DC2626] uppercase tracking-wider pt-2">Industries</p>
          {industries.map((i) => (
          ...
          <div className="border-t border-[#222222] pt-3 space-y-2">
            ...
            <a href="tel:2812312944" className="block text-sm text-[#DC2626] font-semibold">(281) 231-2944</a>
            <Link href="/get-a-quote/" className="block bg-[#DC2626] text-white text-sm font-semibold px-4 py-2 rounded-lg text-center" onClick={() => setMobileOpen(false)}>
              Get a Free Quote
            </Link>
          </div>
        </div>
